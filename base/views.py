...
def index(request):
    ...
    #! posts = Post.objects.all()  Don't need this since we have the feed_list

    #! User suggestions backend starts here

    all_users = User.objects.all()
    user_following_all = [] #! A list of all users the user is following

    for user in user_following:
        user_list = User.objects.get(username=user.user)
        user_following_all.append(user_list)

    #! filters all users we are not following and remove the logged in user from the list
